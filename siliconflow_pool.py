...
class TPMOptimizer:
    ...
    def can_handle_request(self, key: "SiliconFlowKey", estimated_tokens: int) -> bool:
        ...
        total_usage = usage["current"] + usage["reserved"] + estimated_tokens

        return total_usage <= key.tpm_limit
...
class SiliconFlowKey:
    ...
    key: str
    rpm_limit: int = 100  # 每分钟请求数限制
    tpm_limit: int = 10000  # 每分钟token数限制
    is_active: bool = True
    last_used: float = 0
    error_count: int = 0
    success_count: int = 0
        ...
        if os.path.exists(Config.AUTH_KEYS_FILE):
            try:
                with open(Config.AUTH_KEYS_FILE, "r") as f:
                    data = json.load(f)
                    self.auth_keys = set(data.get("keys", []))
                    logger.info(f"Loaded {len(self.auth_keys)} auth keys")
            except Exception as e:
                logger.error(f"Failed to load auth keys: {e}")
        else:
            # 创建默认认证文件
            default_key = (
...
class SiliconFlowPool:
                ...
                with open(Config.API_KEYS_FILE, "r") as f:
                    ...
                    for key_data in data:
                        if isinstance(key_data, str):
                            ...
                            # 确保新字段有默认值
                            if "tpm_limit" not in key_data:
                                key_data["tpm_limit"] = 10000
                            if "total_tokens_used" not in key_data:
                                key_data["total_tokens_used"] = 0
                            self.keys.append(SiliconFlowKey(**key_data))
                    logger.info(f"Loaded {len(self.keys)} SiliconFlow keys")
            except Exception as e:
    ...
    async def save_keys(self):
        ...
        async with self._keys_lock:
            try:
                ...
                logger.debug(f"Saved {len(self.keys)} keys to {Config.API_KEYS_FILE}")
            except Exception as e:
    ...
    def _add_single_key(
        self, key: str, rpm_limit: int = 100, tpm_limit: int = 10000
    ) -> SiliconFlowKey:
        """添加单个 API Key 的内部方法"""
    ...
    def add_keys_batch(
        self, keys_text: str, rpm_limit: int = 100
    ) -> Dict[str, List[str]]:
        """批量添加 API Keys"""
    ...
    def _check_rate_limit(self, key: SiliconFlowKey, estimated_tokens: int = 0) -> bool:
        ...
        # 重置token计数（每分钟重置）
        if current_time - key.last_token_reset >= 60:
            key.tokens_used_this_minute = 0
            key.last_token_reset = current_time

        # 清理1分钟前的请求记录
        self.request_counts[key_id] = [
            t for t in self.request_counts[key_id] if current_time - t < 60
        ]

        # 检查RPM限制
        if len(self.request_counts[key_id]) >= key.rpm_limit:
            return False

        # 检查TPM限制
        if key.tokens_used_this_minute + estimated_tokens > key.tpm_limit:
            return False
    ...
    async def video_generation(self, request_data: Dict, auth_key: str) -> Dict:
        ...
        try:
            ...
            # 添加可选参数
            optional_params = ["image_size", "duration", "fps", "aspect_ratio"]
            ...
            async with self.session.post(
                ...
                if response.status != 200:
                    ...
                    raise HTTPException(
                        ...
                        detail=f"Failed to submit video job: {error_text}",
                    )
                ...
                if not request_id:
                    raise HTTPException(
                        status_code=500,
                        detail="Failed to get requestId from submit response.",
                    )
            ...
            while time.time() - start_time < max_wait_time:
                ...
                async with self.session.post(
                    ...
                    if status == "Succeed":
                        ...
                        # 修复 BUG：增加模型使用统计
                        model = request_data.get("model", "unknown_video_model")
                        self.usage_stats.requests_by_model[model] += 1

                        videos = status_data.get("results", {}).get("videos", [])
                        return {
                            "created": int(time.time()),
                            "data": [{"url": v.get("url")} for v in videos],
                        }
                        ...
                        raise HTTPException(
                            status_code=500, detail=f"Video generation failed: {reason}"
                        )

            # 超时失败
            self.usage_stats.failed_requests += 1  # 添加失败请求统计
            raise HTTPException(
                status_code=408, detail="Video generation timed out after 5 minutes."
            )
    ...
    def get_key_by_id(self, key_id: str) -> Optional[SiliconFlowKey]:
        """根据密钥ID获取密钥对象"""
        for key in self.keys:
...
async def usage_guide():
    ...
    return {
        ...
        "rate_limits": {
            "rpm": "每个API密钥每分钟100次请求",
            "tpm": "每个API密钥每分钟10000个token",
...
async def toggle_key(
    ...
    for key in pool.keys:
        if hashlib.md5(key.key.encode()).hexdigest()[:8] == key_id:
            ...
            await pool.save_keys()  # 修复：使用 await
            return {"success": True, "key_id": key_id, "is_active": key.is_active}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--port", type=int, default=8700, help="Port to bind (default: 8700)"
    )
    parser.add_argument(
